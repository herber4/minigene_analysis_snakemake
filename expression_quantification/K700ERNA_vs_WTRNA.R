...
stats <- ss_ratio %>%
  group_by(sample) %>%
  summarise(
    values = list(cur_data()),
    .groups = "drop"
  ) %>%
  rowwise() %>%
  mutate(
    t_test = list(t.test(C3SS_ratio ~ genotype, data = values)),
    
    t_statistic = as.numeric(t_test$statistic),
    df = as.numeric(t_test$parameter),
    p_value = t_test$p.value,
    
    # optional but very nice for reporting
    mean_MT = t_test$estimate[grep("MT", names(t_test$estimate))],
    mean_WT = t_test$estimate[grep("WT", names(t_test$estimate))]
  ) %>%
  ungroup() %>%
  select(sample, t_statistic, df, p_value, mean_MT, mean_WT) %>%
  mutate(
...
write.table(final_results, file = "K700ERNA_vs_WTRNA_Stats_Table.txt",
